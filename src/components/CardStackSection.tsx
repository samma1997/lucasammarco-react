...
export default function CardStackSection() {
  ...
  const trackRef = useRef<HTMLDivElement>(null) // pinned outer track
  const listRef = useRef<HTMLDivElement>(null)  // scrolling card row
  ...
  useEffect(() => {
    ...
    // Mobile: simple vertical layout — no horizontal scroll GSAP needed
    ...
    const ctx = gsap.context(() => {
      ...
      // We need the list to slide left by (listWidth - viewportWidth)
      ...
      // ---- Card parallax / rotation (skipped if prefers-reduced-motion)
      if (!prefersReduced) {
      ...
      // ---- Intro fade out when first card starts entering ---------------
      if (cards[0]) {
        gsap.to(intro, {
          autoAlpha: 0,
          ease: 'none',
      ...
      // ---- Outro fade in when last card is near right edge --------------
    ...
    >
      ...
      <div
        ...
        style={{
          position: 'relative',
          overflow: 'hidden',
          // On mobile this just becomes a standard block container
        }}
      >
        {/* Intro text — visible before scrolling begins */}
        <div
          ref={introRef}
          ...
          style={{
            ...
            zIndex: 10,
            maxWidth: '20rem',
            // On mobile we reset this to static
          }}
        ...
        {/* Card row — slides left on scroll (desktop) */}
        <div
          ref={listRef}
          ...
          style={{
            ...
            // Extra leading padding so first card starts after intro text
            paddingLeft: 'clamp(18rem, 38vw, 42rem)',
            // Trailing padding so outro has room
        ...
        {/* Outro — fades in at end of card scroll */}
        <div
          ref={outroRef}
      ...
      <style>{`
        @media (max-width: 767px) {
          /* hide the absolute-positioned intro/outro on mobile */
          .card-stack-intro,
          .card-stack-outro {
            display: none !important;
          }

          /* un-pin the track and reset the list to a grid */
          [data-card-stack-list] {
        ...
        /* mobile section header shown only on mobile */
